...
class STLOverlayTrackerSimple:
    def __init__(self, weights_path, device='auto'):
        """
        Simplified 3D STL overlay tracker that projects 3D models onto video scenes.
    ...
    def create_3d_model_image(self):
        """Create a static image of the 3D model."""
        ...
        if self.stl_vertices is not None and self.stl_faces is not None:
            ...
            triangles = []
            for face in self.stl_faces[:500]:  # Limit to first 500 faces
        ...
        ax.set_ylabel('Y')
        ax.set_zlabel('Z')
        ax.set_title('3D Model - Use these coordinates for point selection')
        
        # Save the image
        plt.savefig('3d_model_preview.png', dpi=150, bbox_inches='tight')
        plt.close()
        
        print("3D model preview saved as '3d_model_preview.png'")
        print("You can open this image to see the 3D model structure")
    
    def select_correspondence_points_simple(self, video_frame):
        """Simplified correspondence point selection with predefined 3D points."""
        print("Starting correspondence point selection...")
        print("Select 3 pairs of corresponding points:")
        print("1. Click on video frame to select points")
        print("2. Choose from predefined 3D points (1-6)")
        print("3. Repeat for 3 pairs total")
        
        # Create 3D model preview
        self.create_3d_model_image()
        
        # Predefined 3D points from the model
        self.available_3d_points = [
            [-0.5, -0.5, 0.0],   # Bottom left
            [0.5, -0.5, 0.0],    # Bottom right  
            [0.0, 0.5, 0.0],     # Top center
            [-0.3, 0.3, 0.2],    # Upper left
            [0.3, 0.3, 0.2],     # Upper right
        ...
        while pair_count < 3:
            ...
            if key == ord('q'):
                ...
                # Map the last selected 2D point to the corresponding 3D point
                point_idx = key - ord('1')
                if (len(self.correspondence_points_2d) > pair_count and 
                    point_idx < len(self.available_3d_points)):
                    self.correspondence_points_3d.append(self.available_3d_points[point_idx])
                    print(f"Paired 2D point {len(self.correspondence_points_2d)} with 3D point {point_idx + 1}")
                    pair_count += 1
    ...
    def on_video_click(self, event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            print("Now press '1'-'6' to select which 3D point this corresponds to")
    ...
    def run_video_file(self, video_path):
        ...
        if not self.select_correspondence_points_simple(frame):
    ...
    def prepare_video_writer(self, video_path, fps, frame_shape):
        ...
        output_filename = f"{results_dir}/{input_filename}_3d_overlay_simple.mp4"
...
def main():
    parser = argparse.ArgumentParser(description='Simple 3D STL Overlay Tracker')
    ...
    # Create tracker
    tracker = STLOverlayTrackerSimple(
