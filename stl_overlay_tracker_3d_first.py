...
import matplotlib
matplotlib.use('TkAgg')  # Use interactive backend for 3D model
...
class STLOverlayTracker3DFirst:
    def __init__(self, weights_path, device='auto'):
        """
        3D STL overlay tracker that shows 3D model first, then video.
    ...
    def show_3d_model_and_select_points(self):
        """Show 3D model and allow point selection."""
        print("Opening 3D model viewer...")
        print("Click on 3 points on the 3D model, then press 'q' to continue")
        ...
        self.ax_3d.set_title('3D Model - Click to select 3 points, then press Q')
        ...
        # Wait for 3 points to be selected
        self.correspondence_points_3d = []
        while len(self.correspondence_points_3d) < 3:
            plt.pause(0.1)
            if plt.get_fignums() == []:  # Window was closed
                break
        
        # Close the 3D viewer
        plt.close(self.fig_3d)
        
        if len(self.correspondence_points_3d) >= 3:
            print(f"Selected 3D points: {self.correspondence_points_3d}")
            return True
        else:
            print("Not enough 3D points selected")
            return False
    ...
    def on_3d_click(self, event):
        ...
        if self.stl_vertices is not None:
            ...
            if closest_idx is not None:
                ...
                if len(self.correspondence_points_3d) >= 3:
                    print("3 points selected! Press 'q' to continue to video selection.")
    ...
    def select_video_correspondence_points(self, video_frame):
        """Select corresponding points on the video frame."""
        print("Now opening video window...")
        print("Click on 3 corresponding points on the video frame")
        
        # Create video window
        cv2.namedWindow('Video - Select corresponding points')
        cv2.setMouseCallback('Video - Select corresponding points', self.on_video_click)
        
        self.current_video_frame = video_frame.copy()
        self.correspondence_points_2d = []
        
        # Show the video frame
        cv2.imshow('Video - Select corresponding points', self.current_video_frame)
        
        # Wait for 3 points to be selected
        while len(self.correspondence_points_2d) < 3:
        ...
        cv2.destroyWindow('Video - Select corresponding points')
        
        if len(self.correspondence_points_2d) >= 3:
            print(f"Selected video points: {self.correspondence_points_2d}")
            return True
        else:
            print("Not enough video points selected")
            return False
    ...
    def run_video_file(self, video_path):
        ...
        # Step 1: Show 3D model and select points
        print("\n=== STEP 1: 3D Model Point Selection ===")
        if not self.show_3d_model_and_select_points():
            print("Failed to select 3D points")
            return
        
        # Step 2: Show video and select corresponding points
        print("\n=== STEP 2: Video Point Selection ===")
        ...
        if not self.select_video_correspondence_points(frame):
            print("Failed to select video points")
            return
        ...
        # Step 4: Process video with overlay
        print("\n=== STEP 4: Processing Video with 3D Overlay ===")
    ...
    def prepare_video_writer(self, video_path, fps, frame_shape):
        ...
        output_filename = f"{results_dir}/{input_filename}_3d_overlay_3d_first.mp4"
...
def main():
    parser = argparse.ArgumentParser(description='3D STL Overlay Tracker - 3D First')
    ...
    # Create tracker
    tracker = STLOverlayTracker3DFirst(
